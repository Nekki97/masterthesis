...
n_files 			= 1 	# 0 for all available (56 XCAT models)
...
image_count 		= 1 	# 0 for all available
...
# Data loader automatically loads masks if there is a folder named like dataset folder but with "_masks" at the end

# If there is no mask folder 
#	--> data are loaded as labels (never used)
# If there is a mask folder containing the same data as the data folder 
# 	--> use xcat_organ_mask to create specific organ masks

XCAT_dataset			= "postVIBE_XCAT"		
patient_dataset			= "patient_liver_nrrd"

xcat_organ_mask			= "liver"

trial_name 				= "more_tests"	# Saves entire trial in folder named like this

# main.py loops over elements in this config
trial_config 			= {1:	{"model"			: "DominikCycleGAN",
							   	 "loss_weights"		: "2",
							   	 "xcat_organ_mask"	: "liver"},

						   2:	{"model"			: "AdvancedCycleGAN",
							   	 "loss_weights"		: "2",
							   	 "xcat_organ_mask"	: "liver"}}
