...
for trialID in cfg.trial_config.keys():

	# Configure current for loop model settings
	cfg.trial_data = {"modelname"			: cfg.trial_config[trialID]["model"],
				  	  "steps_per_epoch"		: 0,
				  	  "training_img_shapes"	: [],
				  	  "loss_weights"		: cfg.trial_config[trialID]["loss_weights"],
				  	  "organ"				: cfg.trial_config[trialID]["xcat_organ_mask"]}
	...
	while True:
		try:
			...
			with tf.device('/CPU:0'):
				...
				for dataset_name in [cfg.patient_dataset,cfg.XCAT_dataset]:
			...
			if cfg.train:
				...
				cfg.trial_data["training_img_shapes"] = [np_train_datasets[0].shape, np_train_datasets[1].shape]
				cfg.trial_data["steps_per_epoch"] = min(np_train_datasets[0].shape[0], np_train_datasets[1].shape[0]) // cfg.batch_size
				...
				model_checkpoint_callback = keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath+"/epoch_{epoch:03d}/checkpoint_file_{epoch:03d}", verbose=1, save_weights_only=True, save_freq=int(cfg.save_period*cfg.trial_data["steps_per_epoch"]))
				...
				with tf.device('/device:GPU:0'):
					history = model.fit(tf.data.Dataset.zip((batched_train_datasets[0], batched_train_datasets[1])),
										...
										steps_per_epoch=cfg.trial_data["steps_per_epoch"])
