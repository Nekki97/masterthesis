			...
			if self.epochs_since_last_save % self.period == 0:
				for k in range(2):
					...
					for i, img in enumerate(self.val_data[k].take(self.num_img)):
						...
						ax[4, i].imshow(thrsh_label_pred, cmap='gray')
					...
					if k==0: 
						...
						np.savez(os.path.join(folder_path,"_generated_data",'genG_after_%03d-of-%03d_epochs.npz'%(epoch+1, cfg.n_epochs)), \
							input_img=img[:,:,0], input_label=img[:,:,1], output_img=prediction[:,:,0], output_label=prediction[:,:,1], thresh_out_label=thrsh_label_pred)
